...
const Home = () => {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAllPosts(result.data.reverse());//to show newst post;
      }
    } catch (err) {
      alert(err);
    } finally {
  ...
  const handleSearchChange=(e)=>{
    ...
    setSearchTimeout(
      setTimeout(()=>{
        const searchResult= allPosts.filter((item)=> item.name.toLowerCase().include(searchText.toLowerCase())||item.prompt.toLowerCase().includes(searchText.toLowerCase()));
